...
def insert_into_database():
    ...
    conn = sqlite3.connect('myExperiences.db')
    cursor = conn.cursor()
    ...
    try:
        # Execute the command and commit changes to the database
        cursor.executescript(sql_command_1)
        conn.commit()
    ...
    try:
        # Execute the command and commit changes to the database
        cursor.executescript(sql_command_2)
        conn.commit()
        print("Data inserted successfully!")
    except sqlite3.Error as e:
        print(f"An error occurred: {e}")
